...
namespace Network.Particle.Scripts.Test
{
    public class UnityRPCDemo : MonoBehaviour
    {
        ...
        {
            var json = await EvmService.GetTokensByTokenAddresses("0x529C4A6074Ce9Ea052eCde1AC8E4B935FCd78539", new List<string>{"0x2C89bbc92BD86F8075d1DEcc58C7F4E0107f286b"});
            Debug.Log(json);
        }

        public async void Rpc_eth_getBalance()
        {
            var json = await EvmService.Rpc("eth_getBalance", new List<object>{"0x529C4A6074Ce9Ea052eCde1AC8E4B935FCd78539","latest"});
            Debug.Log(json);
        }
    }
}
